...
while cur_page_num <= target_page_num:
    ...
    for div in div_list:
        ...
        #sql 우선 작성
        #sql을 문자열로 작성하고, 변수가 들어갈 위치를 %s(대부분), %d 등으로 표현
        #값은 tuple에 순서대로 세팅해서 mycursor.execute()에 전달
        query = 'INSERT INTO tbl_crawling (data_rank, title, author, company, publish_date, price) VALUES(%s, %s, %s, %s, %s, %s)' #문자열 %s 정수 %d
        values = (rank, book_title, author_name, company, pub_day, book_price.split(', ')[0]) #마일리지 정보 book_price.split

        mycursor.execute(query, values) #mycursor에 sql 결과값 있음
