...
opener.addheaders = [('User-agent', UserAgent().edge)] #엣지
...
#브라우저 내부 대기
#t.sleep(2) -> 브라우저 로딩에 상관 없이 무조건 10초 대기(cpu 잠재우기)

#웹 페이지 전체가 로딩될 때 까지 대기 후 렌더링이 완료 되었다면 남은 시간 무시
driver.implicitly_wait(10) #암묵적으로 10초 대기

# 엑셀에 텍스트 저장(헤더 만들기)
cell_format = workbook.add_format({'bold': True, 'font_color': 'red', 'bg_color':'yellow', 'font_size': '15'})

#셀 너비 조정
# worksheet.set_column(시작열, 끝 열 번호, 길이)
#셀 높이 조정
...
worksheet.write('C1', '자가', cell_format)
...
for col, header in enumerate(headers): #파이썬 고유 : Pythonic 자주 씀
...
while cur_page_num <= target_page_num:
    ...
    for div_ss_book_box in div_list:
        ...
        '''
        author_name = author.data[0].strip()
        company = author_data[1].strip()
        pub_day = author_data[2].strip()
        '''

        #list comprehension Pythonic 한 코드
        #기존의 리스트를 기반으로 새로운 리스트를 선언하는데,
        #리스트 내부의 요소들에게 일괄적으로 적용할 수식, 조건, 함수의 리턴 값을 동시에 설정할 수 있게 하는 문법
        '''
        li = [1, ,2, 3, 4, 5]
        ...
        try:
            #이미지 바이트 변환 처리
            #BytesIO 객체 생성. 생성자의 매개값으로 아까 준비 해놓은 img 태그의
            #src값을 urlopen 메서드에게 전달
            ...
            #파이썬에서는 블록 구조에 아무 것도 작성하지 않으면 에러
            #블록 구조 내부에 딱히 작성할 코드가 없어서 넘길 때
            #pass라는 키워드를 사용
            
            pass #넘어가(딱힐 쓸 말 없을 때 if else elif : 이후 작성 불가)
