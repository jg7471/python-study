...
#예외 처리를 통해 안전하게 작성하는 것을 권장
try:
    #파일 저장 기능(Write, Append)
    file_path = r'C:\MyWorkspace\upload\test2.txt'

    #파일 입출력을 실행하는 내장 함수 open()
    #반환값으로 파일 입출력을 담당하는 객체가 리턴됨
    #첫번째 인수로 파일 경로를, 두번째 인수로 모드를 설정.
    f = open(file_path, 'a') #w 덮어쓰기 a 추가하기 모드

    text = """
    a모드는 기존의 내용에 추가할 때 사용하는 모드인데,
    만약 파일이 없다면 추가도 해줘요
    """

    #파일을 저장할 때는 wirte() 메서드를 사용
    #저장할 데이터를 인수로 전달
    f.write(text)
    print('파일 저장 완료!')
    ...
    #예외 발생 여부와 상관없이 항상 실행되는 문자를 작성
    #파일 입출력은 하드디스크 자원을 사용하는 코드이므로
    #반드시 사용 후 그 자원을 해제하여 메모리 누수를 방지
    f.close()
