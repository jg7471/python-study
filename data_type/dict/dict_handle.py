...
#해당 key값에 맵핑 되어있는 value가 수정됨.
eng_kor['book'] = '서적'
print(eng_kor)

'''
- 사전의 key 목록과 value 목록을 따로따로 추출하고 싶다면
사전의 메서드 keys(), value()를 사용합니다.'''

print('-' * 40)
print(eng_kor.keys())
print(list(eng_kor.values())) #적절한 자료형(list 등)으로 변환해서 사용 #list 내장 함수 사용

#사전의 반복문 처리
#for ~ in 뒤에 사전 데이터를 적으면 key만 반복 순회
for k in eng_kor:
    print(f'{k} : {eng_kor[k]}')
